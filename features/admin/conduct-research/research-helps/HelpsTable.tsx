        ...
        <TableBody>
          {filtered?.map((help) => (
            <TableRow key={help._id}>
              <TableCell>
                <span className='text-xs mr-1 font-bold'>{help.title}.</span>
                {help.name}
              </TableCell>
              ...
              <TableCell>
                <ConfirmDialog
                  onConfirm={() => deleteMutation.mutate(help._id)}
                  disabled={deletingId === help._id}
                  triggerText={
                    deletingId === help._id ? <ButtonLoader text="Deleting" /> : "Delete"
                  }
                />
              </TableCell>
            </TableRow>
